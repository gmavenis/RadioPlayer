...
unsigned char* aacda_read_buffer( AACDArrayInfo *ainfo )
{
    ...
    if (javaABR.clazz == NULL)
    {
        ...
        javaABR.next = (*env)->GetMethodID( env, javaABR.clazz, "next", "()Lcom/biophysics/radioplayer/ArrayBufferReader$Buffer;");

        javaABR.bufferClazz = (*env)->FindClass( env, "com/biophysics/radioplayer/ArrayBufferReader$Buffer");
